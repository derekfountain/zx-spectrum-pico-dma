...
work with. I wondered if it was possible for a modern microcontroller to do it.
Hence this project.
...
as a test platform. The fundamental problem is that an RP2040-based Pico doesn't
have enough GPIOs to do this sort of task. (It has 26, the Z80's address and data
buses immediately account for 24 of them.) I got it to work, but synchronising
the dual Picos was too slow and fussy. I eventually gave up on that and designed
a new Spectrum interface based on the RP2350B microcontroller.
...
interesting one. So let's start there.

## RP2350B Implementation

My second attempt at getting this to work involved a Raspberry Pi RP2350B microcontroller,
which has 48 GPIOs. Unlike the original RP2040, this is plenty enough for this sort of
project. I used a [Pimoroni Stamp XL](https://shop.pimoroni.com/products/rp2350-stamp-xl?variant=53521688002939).
...
![alt text](images/rp2350_detail.jpg "Board in ZX")
...
The RP2350 can handle the Spectrum's 5V signals as direct inputs to the GPIOs, so
the design is just to link pretty much everything on the Spectrum's rear edge connector
to a GPIO pin. The rest can be done in software.

### The Contention Problem

One of the primary reasons this DMA approach has rarely, if ever, been seen on a
...
to read or write to the screen memory, that device needs to stay clear of
...
* RP2350 will raise a bus request, then wait for the Z80 to yield the buses and
respond with bus acknowledge
* RP2350 will put the address 0x4000 on the address bus
* RP2350 will put the value 0x55 on the data bus
* RP2350 will drive the signal lines in sequence to force a memory write
* RP2350 will then release the bus request signal and the Z80 can go back to
doing whatever it was doing

Compared the dual-Pico approach (scroll down if you want to know) this is pretty
...
of 35 RP2350 "NOP" instructions between the /WR signal going active and inactive
is the minium requirement for successful writes. On a 150MHz RP2350 35 of the
single-cycle "NOP" instructions complete in 233ns, which seems a slower than it
needs to be. I need to investigate this. My test Spectrum for this project has been
one with a static RAM-based lower memory module, so it's hard to draw conclusions
from this hardware.
...
### Transferring a Screenful of Data

The question that needed answering here was whether the DMA transfer would be fast
enough to move a whole ZX Spectrum display frame of data (6,912 bytes) from the RP2350
into the Spectrum's display memory in the time it takes the ULA to draw the top border.
This is where the dual-Pico approach (described below) had failed, and which I needed
to work if this DMA idea was going to be useful.

I boosted the transfer from 1 byte to 6,912 bytes and looked at the timings on the
oscilloscope. It looks like this:

![alt text](images/rp2350_fullscreen.png "Full screen trace")

The blip on the yellow line is the ULA's INT signal indicating a new frame is starting.
The cyan line is the BUSREQ signal as the RP2350 requests the Z80's bus, and the purple
line is the Z80's BUSACK signal showing the Z80 has yielded the bus. The dark blue line
is a test signal which the RP2350 is waggling as each of the 6,912 bytes is fed to the
Spectrum's lower memory. The entire duration of the transfer is about 2.3ms, which is
easily inside the 4.1ms the ULA takes to draw the top border.

## Success!

This is excellent news! Using the RP2350, with all its GPIOs, means it's easily possible
to DMA transfer a whole screen of data, flicker and contention free, from the 
microcontroller into the Spectrum inside the time it takes to draw the top border.

The next step will be to utilise this DMA transfer as part of a ZX co-processor. I have
this vague idea that a RP2350 can be used to do the sorts of things the ZX Spectrum finds
very hard, such as full screen scrolling. That's a different project, but in the meantime
I added a 50-frames-per-second, full screen scroll routine into the RP2350 code, just to
show it's possible. Here's a terribly wobbly (phone in one hand, typing on the rubber keys
with the other) animated GIF showing that scroll working:

![alt text](images/dma.gif "Animated GIF")

## ZX Diagnostics Board Implementation

**TLDR: I got DMA working via a variation of my ZX Diagnostics Board which consists
of 2 Raspberry Pi Picos. But it was slow: with one Pico controlling the address bus,
and the other Pico controlling the data and control buses, a lot of time was wasted
due to the 2 Picos needing to keep sync with one another. So although it worked, it
wasn't really a usable solution. What follows is for the enthusiastic reader only.**

My first attempt at getting this working used my [ZX Diagnostics Board](https://github.com/derekfountain/zx-spectrum-diagnostics) project
